...
export default function LeadsWorkspace() {
  ...
  const { data: leadsData, isLoading, isError } = useQuery({
    queryKey: ['leads-workspace', searchTerm, assigneeFilter, limit, offset],
    queryFn: async () => {
  ...
  // 2. Fetch staff members for Lead Owner select dropdown
  ...
  const metrics = useMemo(() => {
    ...
    let totalCost = 0
    items.forEach((item) => {
      totalCost += Number(item.lead_acquisition_cost || 0)
    })
  ...
  const createLeadMutation = useMutation({
    ...
    onError: (err: any) => {
      const errMsg = err?.response?.data?.detail || err?.message || 'Failed to create lead'
      setFormError(errMsg)
    }
  })
  ...
  const columns = useMemo(() => [
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-zinc-500 truncate">
              {info.row.original.job_title || 'Acquisition Prospect'}
            </div>
          </div>
        </div>
      )
    ...
    {
      accessorKey: 'lead_acquisition_cost',
      header: 'Acquisition Cost',
      cell: (info: any) => {
        const cost = Number(info.getValue() || 0)
        return (
          <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold">
  ...
  const table = useReactTable({
    ...
    columns,
    pageCount: Math.ceil((leadsData?.count || 0) / pagination.pageSize),
    state: { pagination },
  ...
  const paginationCount = leadsData?.count || 0
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Capture, assign, and manage incoming business acquisition leads</p>
        </div>
        <button
          onClick={() => {
            setFormError('')
          ...
          className="flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-semibold transition-all shadow-lg shadow-indigo-600/20 cursor-pointer w-full sm:w-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 flex items-center justify-between">
          <div>
            <span className="text-xs text-zinc-400 font-semibold uppercase tracking-wider block">Total Active Leads</span>
            <span className="text-2xl font-black text-white mt-1 block">{totalLeads}</span>
          </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 flex items-center justify-between">
          <div>
            <span className="text-xs text-zinc-400 font-semibold uppercase tracking-wider block">Avg Acquisition Cost</span>
            <span className="text-2xl font-black text-emerald-400 mt-1 block">
              ₦{metrics.avgCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 flex items-center justify-between">
          <div>
            <span className="text-xs text-zinc-400 font-semibold uppercase tracking-wider block">Page Acquisition Spend</span>
      ...
      <div className="bg-zinc-950/40 border border-zinc-900 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-zinc-950 pl-10 pr-4 py-2 rounded-lg border border-zinc-900 focus:border-indigo-600 focus:outline-none text-sm text-zinc-200 placeholder-zinc-500"
          />
        </div>

        {/* Lead Owner Filter Dropdown */}
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-1.5 bg-zinc-950 px-3 py-2 rounded-lg border border-zinc-900 text-xs w-full md:w-auto">
            ...
            >
              <option value="" className="bg-zinc-950">All Lead Owners</option>
              {usersData?.map((user) => (
      ...
      <div className="bg-zinc-950/40 border border-zinc-900 rounded-xl overflow-hidden shadow-xl">
        {isLoading ? (
          <div className="py-20 text-center text-zinc-400 flex flex-col items-center gap-2">
            ...
            <span className="text-xs">Loading leads directory...</span>
          </div>
        ) : isError ? (
          <div className="py-20 text-center text-red-400 flex items-center justify-center gap-2">
            <AlertCircle className="h-5 w-5" />
            <span className="text-xs">Failed to fetch leads. Please check connection.</span>
          </div>
          ...
          <div className="py-20 text-center text-zinc-500 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-semibold text-sm text-zinc-300">No active leads found</p>
            <p className="text-xs text-zinc-500">Click "+ Add Lead" to record your first prospect</p>
          </div>
        ) : (
          <>
            ...
            <div className="px-6 py-4 border-t border-zinc-900 flex justify-between items-center text-xs text-zinc-400">
              <div>
                ...
                <span className="font-semibold text-zinc-200">{paginationCount}</span> leads
              </div>
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-zinc-900 flex justify-between items-center bg-zinc-900/40">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-md font-bold text-white">Create New Lead</h3>
              </div>
              <button
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    value={formData.lead_acquisition_cost}
                    onChange={handleInputChange}
                    placeholder="5000.00"
                    className="w-full bg-zinc-900 border border-zinc-800 focus:border-indigo-600 focus:outline-none rounded-lg p-2.5 text-sm text-zinc-200 placeholder-zinc-600"
                  />
